...
namespace CliCountry.Facturacion.Web.WebExterno.Facturacion
{
    ...
    using CliCountry.Facturacion.Web.WebExterno.Facturacion.ClasesVinculacion;
    ...
    public partial class AdministradorFacActividad : WebPage
    {
        ...
        protected void BtnCerrarPopupExclusiones_Click(object sender, EventArgs e)
        {
            ...
            VinculacionesFACActividad.Instancia.ObtenerSeleccionada(VinculacionesDeAtencion, VinculacionSeleccionada).NoMarcadas = exclusionesNoMarcadas;
        }
        ...
        private Vinculacion BuscarVinculacion(int identificadorEntidad, int ordenVinculacion)
        {
            var vinculacion = from
                                  vinculacionAtencion in VinculacionesDeAtencion
                              where
                                  vinculacionAtencion.Tercero.Id == identificadorEntidad
                                  && vinculacionAtencion.Orden == ordenVinculacion
                              select
                                  vinculacionAtencion;
        ...
        {
            var Depositos = WebService.Facturacion.ConsultarDepositos(
                new Atencion()
                {
                    IdAtencion = Atencion.IdAtencion
                });

            if (Depositos.Ejecuto)
            {
                Atencion.Deposito = Depositos.Objeto.FirstOrDefault();
                ...
                var Conceptos = WebService.Facturacion.ConsultarConceptos(
                new Atencion()
                {
                    IdAtencion = Atencion.IdAtencion
                });

                if (Conceptos.Ejecuto)
                {
                    if (Atencion.Deposito != null)
                    {
                        Atencion.Deposito.Concepto = Conceptos.Objeto.ToList();
                    }
                }
                else
                {
                    MostrarMensaje(Conceptos.Mensaje, TipoMensaje.Error);
                }
            }
            else
            {
                MostrarMensaje(Depositos.Mensaje, TipoMensaje.Error);
            }
        }
        ...
        {
            var Formatos = WebService.Integracion.ConsultarFormatos();

            if (Formatos.Ejecuto)
            {
                DdlFormato.DataSource = Formatos.Objeto;
            ...
            {
                MostrarMensaje(Formatos.Mensaje, TipoMensaje.Error);
            }
        }
        ...
        private void CargarSecciones()
        {
            ...
            var secciones = WebService.Integracion.ConsultarSecciones(seccion);

            if (secciones.Ejecuto)
            {
                ddlSeccion.DataSource = secciones.Objeto;
            ...
            {
                MostrarMensaje(secciones.Mensaje, TipoMensaje.Error);
            }
        }
        ...
        private void CargarTiposMovimiento()
        {
            ...
            var TiposMovimientos = WebService.Integracion.ConsultarTiposMovimiento(tipoMovimiento);

            if (TiposMovimientos.Ejecuto)
            {
                ddlTipoMovimiento.DataSource = TiposMovimientos.Objeto;
            ...
            {
                MostrarMensaje(TiposMovimientos.Mensaje, TipoMensaje.Error);
            }
        }
        ...
        private void CargarVentas(int identificadorAtencion, List<int> ventasMarcadas)
        {
            ...
            var VentasAtencion = WebService.Facturacion.ConsultarVentasAtencion(parametrosConsulta);
            bool cancelMsg = false;

            if (VentasAtencion.Ejecuto)
            {
                Ventas = VentasAtencion.Objeto.Item;
                ...
                if (!string.IsNullOrEmpty(idatencion))
                {
                    if (idatencion != txtAtencion.Text.Trim())
                    {
                        ...
                        var resultadoBloqueo = WebService.Facturacion.ActualizarBloquearAtencion(VinculacionSeleccionada.IdAtencion, Context.User.Identity.Name);

                        if (!resultadoBloqueo.Ejecuto)
                        {
                            MostrarMensaje("No se pudo desbloquear atención anterior. Consulte administrador para desbloquear.", TipoMensaje.Error);
                        }
                    }
                ...
                if (Ventas != null)
                {
                    string validarBloqueoAtencion = WebService.Facturacion.ValidarAtencionBloqueada(identificadorAtencion);

                    if (string.IsNullOrEmpty(validarBloqueoAtencion))
                    {
                        var resultadoBloqueoAtencion = WebService.Facturacion.ActualizarBloquearAtencion(identificadorAtencion, Context.User.Identity.Name);

                        if (resultadoBloqueoAtencion.Ejecuto && resultadoBloqueoAtencion.Objeto != 0)
                        {
                        ...
                        {
                            if (string.IsNullOrWhiteSpace(resultadoBloqueoAtencion.Mensaje))
                            {
                                MostrarMensaje(Resources.GlobalWeb.FacturacionActividades_ErrorAtencionBloqueada, TipoMensaje.Error);
                            }
                            else
                            {
                                MostrarMensaje(resultadoBloqueoAtencion.Mensaje, TipoMensaje.Error);
                            }
                        }
                    }
                    else if (!string.IsNullOrEmpty(validarBloqueoAtencion) && validarBloqueoAtencion != Context.User.Identity.Name)
                    {
                        consultar = false;

                        string[] rolesDesbloquearAtencion = Settings.Default.UsuarioRoles_DesbloquearAtencion.ToString().Split(',');

                        string usuario = Context.User.Identity.Name;

                        bool autorizado = false;

                        foreach (string rol in rolesDesbloquearAtencion)
                        {
                        ...
                        if (autorizado)
                        {
                            ...
                            lblConfirmacion.Text = "El número de atención: " + identificadorAtencion +
                                " esta bloqueada por: " + validarBloqueoAtencion + ".";
                            mpeConfirmar.Show();
                        }
                    }
                    else if (!string.IsNullOrEmpty(validarBloqueoAtencion) && validarBloqueoAtencion == Context.User.Identity.Name)
                    ...
                    {
                        MostrarMensaje("El número de atención: " + identificadorAtencion + " esta bloqueada por: " + validarBloqueoAtencion, TipoMensaje.Error);
                        LimpiarDatosBasicos();
                    }
                }
                else
            ...
            {
                MostrarMensaje(VentasAtencion.Mensaje, TipoMensaje.Error);
            }
        }
       ...
       private ProcesoFactura ConstruirEncabezadoProcesoFactura()
        {
            ...
            var procesoFactura = new ProcesoFactura()
            {
                ...
                VentasNoMarcadas = VentasFACActividad.Instancia.ObtenerVentasNoMarcadas(grvVentas.Rows.Cast<GridViewRow>())
            };
        ...
        private void GenerarEstadoCuenta() 
        {
            if (Page.IsValid)
            {
                ...
                string delayMostrarReporte = DateTime.Now.Year.ToString() + DateTime.Now.Month.ToString() + DateTime.Now.Day.ToString() + DateTime.Now.Minute.ToString() + DateTime.Now.Second.ToString() + DateTime.Now.Millisecond.ToString();
                VinculacionSeleccionada = VinculacionesFACActividad.Instancia.BuscarVinculacion(Convert.ToInt32(identificadorEntidad.Text), indiceFila + 1, VinculacionesDeAtencion);
                
                grvEntidades.SelectedIndex = 0;
                ActivarCeldaGridView(grvEntidades, indiceFila);

                var ventasSeleccionadas = VentasFACActividad.Instancia.ObtenerVentasMarcadas(grvVentas.Rows.Cast<GridViewRow>());

                if (ventasSeleccionadas.Count() > 0)
                {
                    var procesoFactura = ConstruirEncabezadoProcesoFactura();
                    ...
                    ActivarConceptosXDepositoFactura();
                    ...
                    System.Web.UI.ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "Reporte", String.Format("ecm = window.open(\"{0}?delay={1}\",'_blank');", Resources.AdministradorFacActividades.FacActividad_ReporteEstadoCuenta, delayMostrarReporte), true);
        ...
        private void ActivarConceptosXDepositoFactura()
        {
            if (Atencion.Deposito != null)
            {
        ...
        private List<Vinculacion> ObtenerVinculacionesSeleccionadas()
        {
            ...
            {
                var VinculacionesSeleccionadas = (from fila in grvEntidades.Rows.Cast<GridViewRow>()
                ...
                var Observaciones = (from fila in grvEntidades.Rows.Cast<GridViewRow>()
                                          where item.Contrato.Id == Convert.ToInt32((fila.FindControl(LABELIDCONTRATO) as Label).Text)
                                          && item.Tercero.Id == Convert.ToInt32((fila.FindControl(LABELIDENTIDAD) as Label).Text)
                                          select fila).ToList();

                if (VinculacionesSeleccionadas.Count > 0)
                {
                    item.IndGenerar = 1;
                    resultado.Add(item);
                }

                var VinculacionesNoSeleccionadas = (from fila in grvEntidades.Rows.Cast<GridViewRow>()
                                                  where (fila.FindControl(CHECKGENERAR) as CheckBox).Checked == false
                                                  && (fila.FindControl(CHECKACTIVO) as CheckBox).Checked
                                                  && item.Contrato.Id == Convert.ToInt32((fila.FindControl(LABELIDCONTRATO) as Label).Text)
                                                  && item.Tercero.Id == Convert.ToInt32((fila.FindControl(LABELIDENTIDAD) as Label).Text)
                                                  select item).ToList();
                if (VinculacionesNoSeleccionadas.Count > 0)
                {
                    item.IndGenerar = 0;
                    resultado.Add(item);
                }

                foreach (GridViewRow fl in Observaciones)
                {
        ...
        private void PreSeleccionarSeccion()
        {
            ...
            {
                var seccionPorDefecto = (from
                                       seccion in secciones
                                   where
                                       seccion.CodEsor == Resources.ValoresPreSeleccion.FacRelacion_CodEsor
                                   select
                                       seccion).SingleOrDefault();

                if (seccionPorDefecto != null)
                {
                    ddlSeccion.SelectedIndex = secciones.IndexOf(seccionPorDefecto) + 1;
                }
            }
        }
        ...
        private void PreSeleccionarTipoMovimiento()
        {
            ...
            {
                var movimientoPorDefecto = (from
                                       movimiento in movimientos
                                   where
                                       movimiento.Id == int.Parse(Resources.ValoresPreSeleccion.FacRelacion_TimIde)
                                   select
                                       movimiento).SingleOrDefault();

                if (movimientoPorDefecto != null)
                {
                    ddlTipoMovimiento.SelectedIndex = movimientos.IndexOf(movimientoPorDefecto) + 1;
                }
            }
        }
        ...
        {
            var ventas = VentasFACActividad.Instancia.ObtenerVentasMarcadas(grvVentas.Rows.Cast<GridViewRow>());
